...
`deep_note.md` is the shared content base. `asset_manifest.json` plus the reviewed `assets/` directory is the shared evidence/visual base. They are not finished outputs. PDF, article HTML, and presentation HTML each need their own organization, asset placement, and style treatment before final rendering.
...
3. Segment and analyze.
   ...
   - Before writing the final `deep_note.md`, read `references/deep-note-blog-prompt.md` and use it as the default writing prompt unless the user provides a newer prompt for that run.
...
6. Render selected output(s).
   ...
   - `article-html-only`: render a single-file article HTML from `deep_note.md`, `asset_manifest.json`, selected assets, and `article_render_brief.md`.
   ...
   - For article HTML, prefer the Beautiful Article / Reacticle project structure when that skill is available: `Article.tsx` as assembler, one section per file, `Image` components for selected frames/figures, and `Raw` blocks for diagrams/tables that need custom layout. Do not hand-roll a separate static renderer unless the Reacticle build path is unavailable and the fallback is explicitly labeled.
...
| Article HTML | long-form reading and sharing | `deep_note.md`, `asset_manifest.json`, `article_render_brief.md`, article assets |
...
- Depth and writing defaults: `6C` super-depth analysis and `7A` Blog-style DeepNote. Long/dense videos should become teaching material with argument reconstruction, mechanisms, examples, caveats, and transferable frameworks. The body should be written as a Blog article, not a fixed-length summary; expand dense sections instead of using repeated four-line subsection templates.
...
- Article HTML defaults: `10A` and `11A`. Prefer Beautiful Article / Reacticle when available and auto-select the theme by content. Do not hand-roll a separate static renderer unless the Reacticle path is unavailable and the fallback is explicitly labeled.
